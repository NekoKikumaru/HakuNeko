import cloudscraper, inquirer, json, os, threading
...
class HakuNeko:
    ...
    def hakuneko(self, comics, extension):
        ...
        f = open('hakuneko.mangas.'+extension, 'w+', encoding='iso-8859-8')
        f.write(json.dumps(comics))
        f.close()
        print('hakuneko.mangas.'+extension+' is in '+os.getcwd())
    ...
    def scrape(self, pages):
        for page in pages:
            ...
            while result == 0:
                ...
                for a in aList:
                    self.comics.append({'id': a['href'], 'title': a['title']})
                result = len(aList)

    def bg_thread(self, pages):
        threads = []
        chunks = 10
        for p in range(0, len(pages), chunks):
            t = threading.Thread(target=self.scrape, args=[pages[p:p+chunks]])
            threads.append(t)
        for t in threads:
            t.start()
        for t in threads:
